...
func validateAndReplaceDataWithAttribute(val string, attributes map[string]string) (string, error) {
	matches := globalAttributeRegex.FindAllStringSubmatch(val, -1)
	for _, match := range matches {
		attrValue, ok := attributes[match[1]]
		if !ok {
			return "", fmt.Errorf("Attribute with key %q does not exist", match[1])
		}
